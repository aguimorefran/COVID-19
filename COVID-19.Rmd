...
* **coronavirus** para obtener datos globales actualizados.

# Datasets

* **Datadista**: https://github.com/datadista/datasets
* **WDICountry**: http://databank.worldbank.org/data/download/WDI_csv.zip
...
## Reglas de asociación entre el nivel económico de cada país y la manera en la que el virus ha afectado.
...
df_summary <-coronavirus %>% 
  ...
  arrange(-confirmed) %>%
  filter(confirmed >= 25) %>%
  select(country = country, confirmed, recovered, death) %>%
  mutate(recover_rate = recovered/confirmed,
         death_rate = death/confirmed)

df_summary$recover_rate <- discretize(df_summary$recover_rate, cuts = 3, labels = c("low", "medium", "high"))
df_summary$death_rate <- discretize(df_summary$death_rate, cuts = 3, labels = c("low", "medium", "high"))

WDICountry <- read_csv("datasets/WDICountry.csv")[, c(2, 9)]
WDICountry <- na.omit(WDICountry)
names(WDICountry)[c(1, 2)] <- c("country", "income")
WDICountry$income[WDICountry$income == "High income"] <- "high"
WDICountry$income[WDICountry$income == "Upper middle income"] <- "medium"
WDICountry$income[WDICountry$income == "Lower middle income"] <- "medium"
WDICountry$income[WDICountry$income == "Low income"] <- "low"
merged <- merge(df_summary, WDICountry)
merged <- merged[c(1, 5, 6, 7)]
merged <- column_to_rownames(merged, "country")
merged$income <- as.factor(merged$income)

rules <- apriori(merged, parameter = list(support = 0.1, confidence = 0.5, minlen = 2), appearance = list(lhs=c("income=low", "income=medium", "income=high")))

```

**Con las 3 reglas que obtenemos podemos deducir:**

* `r inspect(rules[1])$lhs` -> `r inspect(rules[1])$rhs` // sup = `r inspect(rules[1])$support`, conf = `r inspect(rules[1])$confidence`.\
Los países que tienen un nivel económico bajo, presentan un ratio de pacientes curados bajo.\
Esto puede deberse a que el sistema sanitario de dichos países tenga carencias.


* `r inspect(rules[2])$lhs` -> `r inspect(rules[2])$rhs` // sup = `r inspect(rules[2])$support`, conf = `r inspect(rules[2])$confidence`.\
Los países que tienen un nivel económico alto, presentan un ratio de pacientes curados alto.\
Se puede deducir que esto se debe a que estos países disponen de un sistema sanitario más avanzado.


* `r inspect(rules[3])$lhs` -> `r inspect(rules[3])$rhs` // sup = `r inspect(rules[3])$support`, conf = `r inspect(rules[3])$confidence`.\
Los países que tienen un nivel económico alto, presentan un ratio de fallecidos alto.\
Tras indagar sobre el tema, he llegado a la conclusión de que esta conclusión probablemente se deba a que los países con mayor nivel económico presentan una esperanza de vida mayor, ya que el virus afecta de manera crítica a los más mayores.