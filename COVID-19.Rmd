...
camas_habitantes %>% 
  select(ccaa, privados_100000, publicos_100000, total_camas_100000) %>% 
  rename(privados = privados_100000,
         publicos = publicos_100000,
         total = total_camas_100000) %>% 
  reshape2::melt(id.vars = "ccaa") %>% 
  filter(variable %in% c("privados", "publicos", "total")) %>% 
  ggplot(aes(x=variable, y=value, fill = variable)) +
  geom_bar(stat = "identity", width = 0.75) + 
  facet_wrap(~ ccaa) + 
  labs(title = "Camas UCI por cada 100,000 habitantes", caption = "Datos proporcionados por datadista") + 
  xlab("") + ylab("Número de camas") + 
